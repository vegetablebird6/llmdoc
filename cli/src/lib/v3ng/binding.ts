...
export async function resolveWriteBinding(options: ResolveWriteBindingOptions): Promise<PreciseBinding> {
  ...
  if (candidates.length === 0) {
    throw new NgError("E_BINDING_NOT_FOUND", "No llmdoc binding exists for this source worktree; write operations require an explicitly established binding", {
  ...
  if (candidates.length > 1) {
    throw new NgError("E_BINDING_AMBIGUOUS", "The registry contains multiple bindings for this source worktree", {
  ...
  if (options.knowledgeInput !== undefined && !sameRealPath(knowledge.worktreeRoot, entry.knowledgeRoot)) {
    throw new NgError("E_BINDING_CONFLICT", "The explicit knowledge root conflicts with the existing binding for this source worktree", {
  ...
  if (config === null) {
    throw new NgError("E_KNOWLEDGE_NOT_INITIALIZED", "The knowledge repository has no llmdoc.yaml; bind and write operations require an llmdoc-initialized knowledge repository", {
  ...
  if (config.repositoryId !== entry.repositoryId) {
    throw new NgError("E_SOURCE_IDENTITY_MISMATCH", "The knowledge repository identity does not match the binding registry", {
      paths: [knowledge.worktreeRoot],
